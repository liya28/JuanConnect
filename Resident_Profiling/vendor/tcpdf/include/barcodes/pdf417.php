...
class PDF417 {
    ...
    $maxecl = 8; // starting max error level
    $maxerrsize = 928 - $numcw; // available codewords for error correction

    // if input $ecl is invalid, assign default based on $numcw
    if ($ecl < 0 || $ecl > $maxecl) {
        if ($numcw < 41) {
            $ecl = 2;
        } elseif ($numcw < 161) {
    ...
    // ensure $ecl is not higher than maximum allowed by available error correction space
    while ($maxecl > 0) {
        // error size is 2^(level + 1), equivalent to 2 << level
        $errsize = 2 << $maxecl;  // this is 2 * 2^maxecl = 2^(maxecl + 1)
                ...
                if (($k = array_search($chval, $this->textsubmodes[$submode])) !== false) {
                    ...
                    for ($s = 0; $s < 4; ++$s) {
                        ...
                        if (($s != $submode) && (($k = array_search($chval, $this->textsubmodes[$s])) !== false)) {
                            ...
                            if (((($i + 1) == $codelen) || ((($i + 1) < $codelen) && (array_search(ord($code[$i + 1]), $this->textsubmodes[$submode]) !== false))) &&
                                (($s == 3) || (($s == 0) && ($submode == 1)))) {
