...
// -----------------------------------------------------------------------------
    ...
    $prev_index = 256; // Clear table marker to indicate fresh start

    while ($data_length >= $bitlen) {
        $index = bindec(substr($bitstring, 0, $bitlen));
        $bitstring = substr($bitstring, $bitlen);
        $data_length -= $bitlen;

        if ($index == 257) { // EOD marker
            break;
        } elseif ($index == 256) { // Clear table marker
            $bitlen = 9;
            $dix = 258;
            $prev_index = 256;
            $dictionary = [];
        ...
        // Increase bit length when dictionary exceeds thresholds
        if ($dix > 2047) {
            $bitlen = 12;
        } elseif ($dix > 1023) {
            $bitlen = 11;
